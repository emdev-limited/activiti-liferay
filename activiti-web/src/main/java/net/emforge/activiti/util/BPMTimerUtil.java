...
public class BPMTimerUtil {
	...
	*/
	public static Date adjust(String offset, Date from) {
		return adjust(0, offset, from);
	}
	
	public static Date adjust(long companyId, String offset, Date from) {
		//Incoming param should looks like +PT10H, so we need to parse it,
				...
				try {
					if (companyId > 0) {
						...
						cal.setTimeZone(c.getTimeZone());
					} else {
						//set default
						cal.setTimeZone(TimeZoneUtil.getDefault());
					}
				} catch (Exception e) {
					_log.warn(String.format("Failed to fetch timezone for company [%s]", companyId));
				}
				_log.debug(String.format("Initial calendar time: [%s]", cal.getTime()));
				if (from != null) {
					cal.setTime(from);
				}
				if (!add) {
				...
				_log.debug(String.format("Adjusted calendar time: [%s]", cal.getTime()));
				return cal.getTime();
	}
