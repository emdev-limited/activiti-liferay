...
public class LoginResource extends ServerResource {
  ...
  public LoginResponse login(LoginInfo loginInfo) {
    if(loginInfo == null) {
      throw new BadRequestException("No login info supplied");
    }
    ...
    if(loginInfo.getPassword() == null) {
      throw new BadRequestException("No password supplied");
    }
    ...
    if (pe != null) {
      if (pe.getIdentityService().checkPassword(loginInfo.getUserId(), loginInfo.getPassword()) == false) {
        throw new NotAuthorizedException("Username and password does not match.");
      }
