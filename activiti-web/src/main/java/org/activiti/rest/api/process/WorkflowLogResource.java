	...
	public ObjectNode addLogEntry(Representation entity) {
		...
		try {
		    ...
		    if (StringUtils.isNotEmpty(logComment)) {
				...
				// get bean
				LiferayTaskService liferayTaskService = (LiferayTaskService)applicationContext.getBean("liferayTaskService");
				liferayTaskService.addWorkflowLogEntry(taskId, task.getProcessInstanceId(), workflowLogEntry);
		    }
		    
		    resultNode.put("success", true);
		} catch (Exception e) {
			throw new ActivitiException("Failed to add workflow log entry", e);
		}
		return resultNode;
	}
