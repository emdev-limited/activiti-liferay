...
public abstract class AbstractPaginateList {
  ...
  public DataResponse paginateList(Form form, Query query,
    ...
    if (sort != null && properties.size() > 0) {
      ...
      if (qp == null) {
        throw new BadRequestException("Value for param 'sort' is not valid, '" + sort + "' is not a valid property");
      }
      ...
      if (order.equals("asc")) {
        ...
        throw new BadRequestException("Value for param 'order' is not valid : '" + order + "', must be 'asc' or 'desc'");
      }
    }
