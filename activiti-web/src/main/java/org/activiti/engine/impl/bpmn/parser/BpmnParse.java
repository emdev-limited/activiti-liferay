...
public class BpmnParse extends Parse {
  ...
  /** A map for storing associations based on their id during parsing. */
  protected Map<String, TransitionImpl> associations;
  ...
  public ProcessDefinitionEntity parseProcess(Element processElement) {
    ...
    associations = new HashMap<String, TransitionImpl>();
      ...
      if(sourceActivity == null && !elementIds.contains(sourceRef)) {
        ...
        if(sourceActivity != null && sourceActivity.getProperty("type").equals("compensationBoundaryCatch")) {
          ...
          if(isForCompensation == null || !(Boolean) isForCompensation) {
        	...
        	String id = associationElement.attribute("id");
        	
        	TransitionImpl transition = sourceActivity.createOutgoingTransition(id);
        	associations.put(id, transition);
        	transition.setProperty("associationDirection", associationElement.attribute("associationDirection"));
    	    transition.setDestination(targetActivity);
        }
      }
    }
  }
  ...
  protected void parseActivity(Element activityElement, Element parentElement, ScopeImpl scopeElement, HashMap<String, Element> postponedElements) {
    ...
    } else if (activityElement.getTagName().equals("textAnnotation")) {
        activity = parseTextAnnotation(activityElement, parentElement, scopeElement);
    }
  ...
  public void parseBPMNEdge(Element bpmnEdgeElement) {
    ...
    if (bpmnElementId != null && !"".equals(bpmnElementId)) {
      TransitionImpl sequenceFlow = null;
      TransitionImpl association = null;
      if (sequenceFlows != null)
        sequenceFlow = sequenceFlows.get(bpmnElementId);
      if (associations != null)
        association = associations.get(bpmnElementId);

      if (sequenceFlow != null || association != null) {
        ...
        if (waypointElements.size() >= 2) {
          ...
          if (sequenceFlow != null)
            ...
            association.setWaypoints(waypoints);
        } else {
        ...
        addError("Invalid reference in 'bpmnElement' attribute, sequenceFlow " + bpmnElementId + "not found", bpmnEdgeElement);
      }
    } else {
  ...
  public ActivityImpl parseTextAnnotation(Element textAnnotationElement, Element parentElement, ScopeImpl scope) {
    ActivityImpl activity = createActivityOnScope(textAnnotationElement, scope);
    Element textElement = textAnnotationElement.element("text");
    activity.setProperty("text", textElement.getText());
    activity.setProperty("textFormat", textElement.attribute("textFormat"));
    return activity;
  }
