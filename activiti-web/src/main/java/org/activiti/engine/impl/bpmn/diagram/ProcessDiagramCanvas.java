...
public class ProcessDiagramCanvas {
  ...
  public void drawNoneEndEvent(int x, int y, int width, int height) {
	  drawNoneEndEvent(null, x, y, width, height);
  }
 ...
 // text
  ...
  public void drawLabel(String text, int x, int y, int width, int height){
    ...
    if (text != null) {
      ...
      int wrapWidth = 100;
      //int textX = x + width/2 - wrapWidth/2;
      ...
      //LineMetrics lm = g.getFont().getLineMetrics(text, frc);

      while (lbm.getPosition() < text.length()) {
    	  TextLayout tl = lbm.nextLayout(wrapWidth);
    	  textY += tl.getAscent();
    	  Rectangle2D bb = tl.getBounds();
    	  float tY = (float) (x  + width/2 - bb.getWidth()/2);
    	  tl.draw(g, tY, textY);
      ...
      //List<String> lines = StringUtils.wrap(text, fontMetrics, width);
      //String newText = ListUtil.toString(lines, STRING_ACCESSOR, "\n");
