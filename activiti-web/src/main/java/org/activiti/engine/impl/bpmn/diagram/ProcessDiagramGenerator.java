...
public class ProcessDiagramGenerator {
  ...
  static {
    ...
    activityDrawInstructions.put("endEvent", new ActivityDrawInstruction() {
      ...
      public void draw(ProcessDiagramCanvas processDiagramCreator, ActivityImpl activityImpl) {
        processDiagramCreator.drawNoneEndEvent((String) activityImpl.getProperty("name"), activityImpl.getX(), activityImpl.getY(), activityImpl.getWidth(), activityImpl.getHeight());
      }
    });
    ...
    // text annotation
    activityDrawInstructions.put("textAnnotation", new ActivityDrawInstruction() {

      public void draw(ProcessDiagramCanvas processDiagramCreator, ActivityImpl activityImpl) {
        processDiagramCreator.drawTextAnnotation((String) activityImpl.getProperty("text"), activityImpl.getX(), activityImpl.getY(),
                activityImpl.getWidth(), activityImpl.getHeight());
      }
    });
  }
  ...
  protected static void drawActivity(ProcessDiagramCanvas processDiagramCanvas, ActivityImpl activity, List<String> highLightedActivities, List<String> highLightedFlows) {
    ...
    for (PvmTransition sequenceFlow : activity.getOutgoingTransitions()) {
      ...
      List<Integer> waypoints = ((TransitionImpl) sequenceFlow).getWaypoints();
      if (waypoints.size() == 0) {
    	  // there isn't any waypoints... Not good.
    	  continue;
      }
      ...
      if ("textAnnotation".equals(activity.getProperty("type"))) {
        String associationDirection = (String) sequenceFlow.getProperty("associationDirection");
  ...
  protected static ProcessDiagramCanvas initProcessDiagramCanvas(ProcessDefinitionEntity processDefinition) {
    ...
    return new ProcessDiagramCanvas(maxX + 50, maxY + 50, minX - 50, minY - 50);
  }
